...
export default function CardAnimationSection() {
  ...
  // Playground rotation state controls (initialized to current design defaults)
  const [rotateX, setRotateX] = useState(58);
      ...
      >
        <style>{`
          ...
          @keyframes itp-pointer-fade-in {
            from { opacity: 0; transform: translate3d(-50%, 15px, 0); }
        ...
        {/* Dynamic Rotation Controls (Desktop only) */}
        {!isMobile && (
          <div className="absolute top-6 left-6 z-[100] bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border border-slate-200 dark:border-white/10 p-4 rounded-2xl shadow-xl flex flex-col gap-3 w-64 pointer-events-auto">
            ...
            <div style={{ position: "absolute", left: "50%", top: "48%", transform: "translate(-50%, -50%)" }}>
              <div
                style={{
                  ...
                  transform: `perspective(3200px) rotateX(${rotateX}deg) rotateZ(${rotateZ}deg) scale(${scale})`,
                ...
                {inView && (
                  <>
                    ...
                    <div className="itp-pointer" style={{ left: 0 + 197, top: 220, animationDelay: '12.0s' }}>
                      <div style={{ transform: `rotate(${-rotateZ}deg) rotateX(${-rotateX}deg)`, transformStyle: "preserve-3d", display: "flex", flexDirection: "column", alignItems: "center" }}>
